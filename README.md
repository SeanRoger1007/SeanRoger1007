👋 Hi, I'm Sean Roger Gacela

🎓 BS Computer Science Graduate
💼 Open to Entry-Level Opportunities (Software Dev | QA | IT Support | Data)

<h2>🚀 About Me</h2>

I'm a Bachelor of Science in Computer Science graduate passionate about building practical solutions and continuously learning new technologies. I enjoy working on projects that improve efficiency, solve real-world problems, and enhance user experience mostly guaranteed satisfaction.

<h2>🔭 Currently learning:</h2> [e.g., React, Python, SQL]
<h2>🌱 Improving skills in:</h2> [e.g., problem-solving, debugging]
<h2>💡 Interests:</h2> [Web Dev, Mobile Apps, AI, Machine Learning, etc.]
<h2>🛠️ Tech Stack</h2>

Languages: Java, Python, C++, JavaScript
Web: HTML, CSS, React 
Mobile: Android Studio, SwiftUI
Database: MySQL
Tools: Git, VS Code, Eclipse, Google Collab

<h2>📈 Skills</h2>
Problem Solving
Team Collaboration
Communication
Time Management

<h2>🏆 Achievements</h2>
[Dean’s Lister / Awards / Certifications]


<h2> 🤳 Connect with me:</h2>

[<img align="left" alt="SeanGacela | Facebook" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/facebook.svg" />][facebook]
[<img align="left" alt="SeanGacela | YouTube" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/youtube.svg" />][youtube]
[<img align="left" alt="SeanGacela | Twitter" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/twitter.svg" />][twitter]
[<img align="left" alt="SeanGacela | LinkedIn" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/linkedin.svg" />][linkedin]
[<img align="left" alt="SeanGacela | Instagram" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/instagram.svg" />][instagram]

[facebook]: https://www.facebook.com/SeanGacela
[twitter]: https://twitter.com/
[youtube]: https://www.youtube.com/
[instagram]: https://www.instagram.com/parmacean
[linkedin]: https://www.linkedin.com/in/sean-roger-gacela/

⭐ I'm actively looking for my first opportunity—feel free to connect or reach out!
